...
Exit 0 iff printed decimal >= B_hi (exact) AND dual-precision nesting holds.
"""
...
def main():
    ...
    assert (Y_arr >= 0).all() and m_arr.min() >= 1 and m_arr.max() <= M
    ...
    if args.cert:
        ...
        print(f"[cross] cert hash match  = {y_hash == c['y_hash_sha256']}")
        print(f"[cross] sum_y frac match = {c['sum_y_exact_fraction'] == [S_Y, D]}")
        cert_ten = Fraction(c["ten_times_sum_abs_r_upper_float"])
    ...
    ok = bool(nest_ok) and (vault_ok is None or bool(vault_ok))
