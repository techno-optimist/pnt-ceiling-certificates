...
def main():
    ...
    p.add_argument("--keys-file", default="")   # json {"keys":[...]} or npz whitelist
    p.add_argument("--tol", type=float, default=1.0001)   # submission-safe: 1.00009999
    args = p.parse_args()

    K = args.K; M = 10 * K; TOL = args.tol
    tag = f"K{K}"
    outdir = args.outdir
    ...
    if args.keys_file:
        if args.keys_file.endswith(".json"):
            wl = json.load(open(args.keys_file))["keys"]
            ks = np.array(sorted({int(k) for k in wl}), dtype=np.int64)
        else:
            dd = np.load(args.keys_file)
            key = "keys" if "keys" in dd.files else dd.files[0]
            ks = np.array(sorted({int(k) for k in dd[key]}), dtype=np.int64)
        ks = ks[(ks >= 2) & (ks <= K)]
        log(f"RESTRICTED whitelist: {ks.size} keys, maxkey={int(ks.max())} TOL={TOL}")
    else:
...
if __name__ == "__main__":
    try:
        ...
        sys.stdout.flush()
        try:
            with open("CRASH_restricted", "w") as fh:
